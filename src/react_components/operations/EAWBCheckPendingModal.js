...
class EAWBCheckPendingModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	eAWBCheckPendingAction = (legop_record) => {
		...
		if (error_count < error_type_count) {
			...
			APIService.post('/eAwbCheckPending', {
				...
				closing_status: custom.custom.awb_leg_ops_status.e_awb_check_done,
				created_by: legop_record.awb_leg.created_by,
				from: legop_record.awb_leg.from,
				CCA: this.state.cca,
				awb_type: this.state.awbType,
				amended_awb: this.state.amendedAwb
	...
	onRadioChange = (event) => {
		this.setState({ awbType: event.target.id });
	}
	onAmendedAwbChange = (event) => {
		
		this.state.amendedAwb = !this.state.amendedAwb;
	}
				...
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="eAWBCheckPendingModalEditTitle">E AWB Check Pending
					...
					<ModalBody>  
						...
						<div>
						<input id='E_AWB' type="radio" name="status" className='mx-2' onClick={this.onRadioChange} checked={this.state.awbType === 'E_AWB'} defaultChecked />
						<label>E-AWB</label>
						<input id='AWB' type="radio" name="status" className='mx-2' onClick={this.onRadioChange} checked={this.state.awbType === 'AWB'}/>
						<label>AWB</label>
						</div>
						<div>
						<input id='amendedAwb' defaultChecked={this.state.amendedAwb} type="checkbox" name="status" className='mx-2' onClick={this.onAmendedAwbChange} />
						<label>Amended AWB</label>
						</div>
						...
						<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"eAWBCheckPendingModalAction"} onClickHandler={() => this.eAWBCheckPendingAction(this.state.legop_record)} text={"E AWB Check Done"}/>
...
export default EAWBCheckPendingModal;