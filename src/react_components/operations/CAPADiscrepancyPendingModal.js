...
class CAPADiscrepancyPendingModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	capADiscrepancyPendingAction = (legop_record) => {
		...
		if (error_count < error_type_count) {
			...
			APIService.post('/capADiscrepancyPending', {
				...
				closing_status: custom.custom.awb_leg_ops_status.cap_a_discrepancy_done,
	...
	render() {
		...
		return (
			<Observer>{() =>
				<div>
					<ModalHeader>
						<h4 className="modal-title">
							...
							<label className="ml-2" id="capADiscrepancyPendingModalEditTitle">CAP A Discrepancy Pending
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"capADiscrepancyPendingModalAction"} onClickHandler={() => this.capADiscrepancyPendingAction(this.state.legop_record)} text={"CAP A Discrepancy Done"}/>
				...
				export default CAPADiscrepancyPendingModal;