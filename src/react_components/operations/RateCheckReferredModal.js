...
class RateCheckReferredModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	rateCheckReferredAction = (legop_record) => {
		let errors = this.state.errors;
		...
		if (!this.state.closing_status) {
			errors.closing_status_error = 'select closing Point';
			this.setState({ errors });
		} else {
		...
		if (error_count < error_type_count) {
			...
			APIService.post('/rateCheckReferred', {
				...
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="rateCheckReferredModalEditTitle">Rate Check Referred
					...
					<ModalBody>  
						...
						}
						{
							...
							<div>
								...
								<input className="col-md-12" id="rateCheckReferredModalVoidReasonInput" placeholder="Enter reason to discard awb leg" type="text" name="rateCheckReferredModalVoidReasonInput" autocomplete="off" required="" onChange={this.void_reason}></input>
								<label className="col-md-12 text-danger" style={{ fontSize: "14px", fontWeight: " 400" }}>
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"rateCheckReferredModalAction"} onClickHandler={() => this.rateCheckReferredAction(this.state.legop_record)} text={"SAVE"}/>
...
export default RateCheckReferredModal;