...
class ReadyToFDCModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	readyToFDCAction = (legop_record) => {
		...
		if (error_count < error_type_count) {
			...
			APIService.post('/readyToFDC', {
				...
				closing_status: custom.custom.awb_leg_ops_status.fdc_pending,
				...
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="readyToFDCModalEditTitle">Ready To FDC
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"readyToFDCModalAction"} onClickHandler={() => this.readyToFDCAction(this.state.legop_record)} text={"SEND TO FDC Pending"}/>
...
export default ReadyToFDCModal;