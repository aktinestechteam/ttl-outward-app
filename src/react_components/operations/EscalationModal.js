...
class EscalationModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	setClosingStatus = (status) => {
		...
		this.setState({ errors: { void_reason_error: '', src_error: '', dest_error: '', selected_flight_error: '', closing_status_error: '' } });
	...
	destinationChanged = (event) => {
		...
		this.updateFlightsEscalationModal(event.target.value);
	}
	...
	updateFlightsEscalationModal = (destination_value) => {
				...
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="escalationModalEditTitle">Escalation
					...
					<ModalBody>  
						...
						}
						{
							this.state.closing_status === custom.custom.awb_leg_ops_status.assign_flight_to_recovery &&
							<div>
								<input className="col-md-12" id="escalationModalVoidReasonInput" placeholder="Enter reason for assigning another flight" type="text" name="escalationModalVoidReasonInput" autocomplete="off" required="" onChange={this.void_reason}></input>
								<label className="col-md-12 text-danger" style={{ fontSize: "14px", fontWeight: " 400" }}>
								...
								<div className="row">
									<div className="col-md-5 input-group">
										...
										<select className="p-0 select3 form-control custom-select browser-default" id="escalationModalStationSourceSelect" type="text">
											<option value={this.state.legop_record.awb_leg.from}>{this.state.legop_record.awb_leg.from}</option>
										</select>
										<select className="p-0 select3 form-control custom-select browser-default" id="escalationModalStationDestinationSelect" type="text" onChange={this.destinationChanged}>
											{
									...
									<div className="col-md-7 input-group">
										...
										<select className="select2 form-control custom-select my-auto" id="escalationModalFlightsSelector" type="text" placeholder="Select flight" name="escalationModalFlightsSelector" onChange={this.flightChanged}>
											{
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"escalationModalAction"} onClickHandler={() => this.escalationAction(this.state.legop_record)} text={"SAVE"}/>
