...
class P1EscalationModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
			...
			errors: { void_reason_error: '', src_error: '', dest_error: '', selected_flight_error: '', closing_status_error: '', closing_ba80_notes_error: '', closing_reason_error: '' },
		}
	}
	...
	void_reason = (event) => {
		...
		let isValidatedP1Escalation = window.validate(rules);
		console.log('isValidatedP1Escalation ' + JSON.stringify(window.isValidatedEscalation));
		if (isValidatedP1Escalation.errmsg) {
			errors.void_reason_error = isValidatedP1Escalation.errmsg;
	...
	setClosingStatus = (status) => {
		...
		this.setState({ errors: { void_reason_error: '', src_error: '', dest_error: '', selected_flight_error: '', closing_status_error: '', closing_reason_error: '', closing_ba80_error: '' } });
	...
	destinationChanged = (event) => {
		...
		this.updateFlightsP1EscalationModal(event.target.value);
	}
	...
	updateFlightsP1EscalationModal = (destination_value) => {
	...
	p1EscalationAction = (legop_record) => {
		let errors = this.state.errors;
				...
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="p1EscalationModalEditTitle">P1 Escalation
					...
					<ModalBody>  
						...
						<RadioOptions options={['ASSIGN_FLIGHT_TO_RECOVERY', 'ESCALATION']} optionSelected={this.setClosingStatus} />
						...
						}
						{
							this.state.closing_status === custom.custom.awb_leg_ops_status.assign_flight_to_recovery &&
							<div>
								<input className="col-md-12" id="p1EscalationModalVoidReasonInput" placeholder="Enter reason for assigning another flight" type="text" name="p1EscalationModalVoidReasonInput" autocomplete="off" required="" onChange={this.void_reason}></input>
								<label className="col-md-12 text-danger" style={{ fontSize: "14px", fontWeight: " 400" }}>
								...
								<div className="row">
									<div className="col-md-5 input-group">
										...
										<select className="p-0 select3 form-control custom-select browser-default" id="p1EscalationModalStationSourceSelect" type="text">
											<option value={this.state.legop_record.awb_leg.from}>{this.state.legop_record.awb_leg.from}</option>
										</select>
										<select className="p-0 select3 form-control custom-select browser-default" id="p1EscalationModalStationDestinationSelect" type="text" onChange={this.destinationChanged}>
											{
									...
									<div className="col-md-7 input-group">
										...
										<select className="select2 form-control custom-select my-auto" id="p1EscalationModalFlightsSelector" type="text" placeholder="Select flight" name="p1EscalationModalFlightsSelector" onChange={this.flightChanged}>
											{
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData}  awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"p1EscalationModalAction"} onClickHandler={() => this.p1EscalationAction(this.state.legop_record)} text={"SAVE"}/>
...
export default P1EscalationModal;