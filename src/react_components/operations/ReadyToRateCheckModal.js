...
import OperationModalLockableFooter from '../shared/OperationModalLockableFooter';
import APIService from "../APIService.js";
class ReadyToRateCheckModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	readyToRateCheckAction = (legop_record) => {
		...
		if (error_count < error_type_count) {
			...
			APIService.post('/readyToRateCheck', {
				...
				closing_status: custom.custom.awb_leg_ops_status.rate_check_pending,
				...
				<div >
					<ModalHeader>
						<h4 className="modal-title">
							...
							<label className="ml-2" id="readyToRateCheckModalEditTitle">Ready to Rate Check
							<NavigationButton awb_no={this.state.legop_record.awb_no} tabNumber={this.state.tabNumber} MainStore={this.props.MainStore} onCloseModal={this.onCloseModal} tabid={custom.custom.tab_name.planned}/>
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"readyToRateCheckModalAction"} onClickHandler={() => this.readyToRateCheckAction(this.state.legop_record)} text={"SEND TO RATE CHECK"}/>
				</div>
...
export default ReadyToRateCheckModal;