...
class CAPAPendingModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
			...
			reason: "",
			ba80_notes: "",
			closing_status: custom.custom.awb_leg_ops_status.cap_a_done,
			checked: true,
			cca: [],
			tabNumber: '',
	...
	cap_a_discrepancy = (event) => {
		console.log("checkbox change " + this.state.checked);
		this.setState({ checked: !this.state.checked });
		if (this.state.checked === true) {
			this.setState({ closing_status: custom.custom.awb_leg_ops_status.cap_a_discrepancy });
		} else {
			this.setState({ closing_status: custom.custom.awb_leg_ops_status.cap_a_done });
		}
		console.log(this.state.closing_status);
	}

	capAPendingAction = (legop_record) => {
				...
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="capAPendingModalEditTitle">CAP A Pending
						...
						<div className="col-lg-10">
							<ul className="list-style-none">
								<li style={{ paddingTop: "1rem", paddingRight: "1rem", paddingLeft: "1rem" }}>
									<label className="customcheckbox alert alert-primary" style={{ fontSize: "16px", fontWeight: " 600" }}>CAP A DISCREPANCY
										<input className="listCheckbox" id="capAPendingModalCAPADiscrepancy" type="checkbox" name="capAPendingModalCAPADiscrepancy" onChange={this.cap_a_discrepancy}  />
										<span className="m-auto checkmark"></span>
									</label>
								</li>
							</ul>
						</div>
					</ModalBody>  
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"capAPendingModalAction"} onClickHandler={() => this.capAPendingAction(this.state.legop_record)} text={"SAVE"}/>
...
export default CAPAPendingModal;