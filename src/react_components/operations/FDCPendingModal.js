...
class FDCPendingModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	void_reason = (event) => {
		...
		let isValidatedFDCPending = window.validate(rules);
		console.log('isValidatedFDCPending ' + JSON.stringify(isValidatedFDCPending));
		if (isValidatedFDCPending.errmsg) {
			errors.void_reason_error = isValidatedFDCPending.errmsg;
			this.setState({ errors });
		} else {
	...
	fdcPendingAction = (legop_record) => {
		let errors = this.state.errors;
		if (this.state.closing_status !== custom.custom.awb_leg_ops_status.fdc_done) {
			...
			<Observer>{() =>
					...
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="fdcPendingModalEditTitle">FDC Pending 
					...
					<ModalBody>  
						...
						<RadioOptions options={['FDC_HOLD', 'FDC_REJECTED', 'FDC_DONE']} optionSelected={this.setClosingStatus} />
						<div>
							<CCAMultiReason ccaCallback={this.callbackCCA} />
						</div>
						{
							this.state.closing_status === custom.custom.awb_leg_ops_status.fdc_done &&
							<div>
							</div>
						}
						{
							...
							<div>
								...
								<input className="col-md-12 my-auto" id="fdcPendingModalVoidReasonInput" placeholder="Enter reason to discard awb leg" type="text" name="fdcPendingModalVoidReasonInput" autocomplete="off" required="" onChange={this.void_reason}></input>
								<label className="col-md-12 text-danger" style={{ fontSize: "14px", fontWeight: " 400" }}>
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"fdcPendingModalAction"} onClickHandler={() => this.fdcPendingAction(this.state.legop_record)} text={"SAVE"}/>
...
export default FDCPendingModal;