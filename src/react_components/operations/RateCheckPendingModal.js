...
import CCAMultiReason from "../shared/CCAMultiReason.js";
class RateCheckPendingModal extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	setClosingStatus = (status) => {
		...
		this.setState({ errors: { void_reason_error: '', offload_error: '', closing_status_error: '' }, closing_status: status });
	}
	...
	rateCheckPendingAction = (legop_record) => {
		let errors = this.state.errors;
		...
		if (error_count < error_type_count) {
			...
			APIService.post('/rateCheckPending', {
	...
	onCloseModal = (event) => {
		...
		//console.log("====reason records====="+reason_records.length);
	}
	

	render() {
		// let modalSize = "xl";
		// console.log('----cca---'+ this.state.selectedCCA);
		return (
			<Observer>{() =>
				<div>
					<ModalHeader>  
						<h4 className="modal-title">
							...
							<label className="ml-2" id="rateCheckPendingModalEditTitle">Rate Check Pending
					...
					<ModalBody>  
						...
						<RadioOptions options={['RATE_CHECK_REFERRED', 'RATE_CHECK_HOLD', 'RMS_REVIEW', 'RATE_CHECK_DONE', 'RATE_CHECK_REJECTED']} optionSelected={this.setClosingStatus} />
						<div>
						...
						}
						{
							...
							<div>
								...
								<input className="col-md-12" id="ratecheckPendingModalVoidReasonInput" placeholder="Enter reason to discard awb leg" type="text" name="ratecheckPendingModalVoidReasonInput" autocomplete="off" required="" onChange={this.void_reason}></input>
								<label className="col-md-12 text-danger" style={{ fontSize: "14px", fontWeight: " 400" }}>
					...
					<OperationModalLockableFooter MainStore={this.props.MainStore} lockedOpsData={this.props.lockedOpsData} awbLegOp={this.props.legop_record} onCloseModal={this.onCloseModal} name={"ratecheckPendingModalAction"} onClickHandler={() => this.rateCheckPendingAction(this.state.legop_record)} text={"SAVE"}/>
...
export default RateCheckPendingModal;