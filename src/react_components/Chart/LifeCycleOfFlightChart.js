...
export default class LifeCycleOfFlightChart extends Component {
    constructor(props) {
        super(props)
    ...
    downloadData = async()=>{
        ...
        APIService.post('/getLifeCycleOfFlightData', req,(res) => {
            this.setState({awbInfos: res});
            if(res.length>0){
                let csvData = res;
                if(csvData.length>0){
                    
                    let fileName = "Life Cycle of Flight"+`${Date.now()}.xls`;
                    const fileType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
                    const fileExtension = '.xlsx';
                    const ws = XLSX.utils.json_to_sheet(csvData);
                    const wb = { Sheets: { 'data': ws }, SheetNames: ['data'] };
    ...
    render() {
        ...
        return (
            <Observer>{()=>
                ...
                <h3 scope="col" className="card-title" colspan="9"><strong>Life Cycle of Flight</strong></h3>
