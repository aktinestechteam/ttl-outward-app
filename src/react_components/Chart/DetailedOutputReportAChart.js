...
export default class DetailedOutputReportAChart extends Component {
    constructor(props) {
        super(props)
    ...
    downloadData = async()=>{
        ...
        req.actions=this.state.selectedAction;
    
        APIService.post('/getDetailedOutputReportAData', req,(res) => {
            this.setState({awbInfos: res});
            if(res.length>0){
                let csvData = res;
                if(csvData.length>0){
                    
                    let fileName = "Detailed Output Report (A)"+`${Date.now()}.xls`;
                    const fileType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
                    const fileExtension = '.xlsx';
                    const ws = XLSX.utils.json_to_sheet(csvData);
                    const wb = { Sheets: { 'data': ws }, SheetNames: ['data'] };
    ...
    render() {
        ...
        return (
            ...
            <div className="card bg-light" style={{"padding": "inherit"}}>
                ...
                <h3 scope="col" className="card-title" colspan="9"><strong>Detailed Output Report (A)</strong></h3>
                    ...
                    <div className="row">
                        <div className="col-md-6 input-group">
                            ...
                            {/* <MultiSelect data={this.state.actions} getList={this.getSelectedActionsList}/> */}
                            <select className="p-0 select3 form-control custom-select browser-default" id="awbKundaliModalStationDestinationSelect" type="text" onChange={this.getSelectedActionsList}>
                            {
