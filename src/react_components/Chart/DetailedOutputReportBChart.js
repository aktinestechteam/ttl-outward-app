...
export default class DetailedOutputReportBChart extends Component {
    constructor(props) {
        super(props)
    ...
    getSelectedActionsList = (list) =>{
        let newList=[]
        for(let i=0;i<list.length;i++){
            newList.push(list[i].value);
        }
		this.setState ({
			selectedAction: newList
		});
	}
    ...
    downloadData = async()=>{
        ...
        req.actions=this.state.selectedAction.length == 0 ? this.state.actions : this.state.selectedAction;
        
        APIService.post('/getDetailedOutputReportBData', req,(res) => {
            console.log(res);
            ...
            if(res.length>0){
                ...
                if(csvData.length>0){
                    ...
                    let fileName = "Detailed Output Report (B)"+`${Date.now()}.xls`;
                    const fileType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
                    const fileExtension = '.xlsx';
                    const ws = XLSX.utils.json_to_sheet(csvData);
                    const wb = { Sheets: { 'data': ws }, SheetNames: ['data'] };
    ...
    render() {
        ...
        return (
            <Observer>{()=>
                ...
                <h3 scope="col" className="card-title" colspan="9"><strong>Detailed Output Report (B)</strong></h3>
