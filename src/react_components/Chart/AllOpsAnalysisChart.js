...
export default class AllOpsAnalysisChart extends Component {
    constructor(props) {
        super(props)
    ...
    downloadData = async()=>{
        ...
        // req.destination=this.state.selectedDestinations;
        
        APIService.post('/getAllOpsperformance', req,(res) => {
            ...
            if(res.length>0){
                ...
                if(csvData.length>0){
                    ...
                    let fileName = "All Ops Performance "+`${Date.now()}.xls`;
                    const fileType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
                    const fileExtension = '.xlsx';
                    const ws = XLSX.utils.json_to_sheet(csvData);
                    const wb = { Sheets: { 'data': ws }, SheetNames: ['data'] };
    ...
    render() {
        ...
        return (
            <Observer>{()=>
                ...
                <h3 scope="col" className="card-title" colspan="9"><strong>Analysis of all the ops processes</strong></h3>
