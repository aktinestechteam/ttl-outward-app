...
export default class EAWBPerformanceChart extends Component {
    constructor(props) {
        super(props)
    ...
    downloadData = async()=>{
        ...
        APIService.post('/getEAWBData', req,(res) => {
            ...
            if(res.length>0){
                ...
                if(csvData.length>0){
                    ...
                    let fileName = "eAWB Performance "+`${Date.now()}.xls`;
                    const fileType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8';
                    const fileExtension = '.xlsx';
                    const ws = XLSX.utils.json_to_sheet(csvData);
                    const wb = { Sheets: { 'data': ws }, SheetNames: ['data'] };
    ...
    render() {
        ...
        return (
            <Observer>{()=>
                ...
                <h3 scope="col" className="card-title" colspan="9"><strong>E-AWB Performance</strong></h3>
